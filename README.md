...
[template.md](template.md) を**8カテゴリ・全55枚**に整理しました。作例は見た目・用途が異なるものに絞り、設定方法はこのREADMEに集約しています。各カテゴリの先頭に扉を置いています。2枚目が全体の目次です。
...
画像を大きく見せる標準例は27枚目です。`figure no-title captioned` と `![h:560 説明](画像のパス)` で、16:9なら約996×560pxに表示し、図表名と外周の余白を確保します。タイトルを出す場合は `no-title` を外し、画像を `h:440` にします。元の縦横比を保って表示するので、4:3や縦長の画像にも使えます。画像に高さ指定を使うこの配置では `ratio-*` を重ねません。

3:1はパノラマや帯状の画像向けです。専用の作例は省きましたが、`figure ratio-3x1` で引き続き利用できます。
...
![h:560 16:9の画像](assets/placeholders/landscape-16x9.svg)

###### 図1：大きな画像の配置
```

本文からは `[表1](#表1プラン比較)` や `[図1](#図1大きな画像の配置)` で参照します。Marpが見出しからリンク先を生成するため、**この例では名前の「：」を除いて指定**します。英字は小文字、空白はハイフンになるなど変換規則があるので、名前を複雑にしないのがおすすめです。
...
カスタマージャーニーマップ、リーンキャンバス、ビジネスモデルキャンバス、SWOT、クロスSWOT、競合分析、施策の優先順位を42〜49枚目にまとめています。すべて架空の予約管理サービスを題材にした例です。

`journey` は段階を列・観点を行にするMarkdownの表、`canvas` は9項目の箇条書きです。SWOTは `cards card-top`、クロスSWOTは `matrix matrix-neutral` を使います。用途だけが違うクラスを増やさず、競合分析は `comparison-table`、施策比較は `dense` を再利用します。

`canvas` の順番は「左の縦長枠 → 左中の上・下 → 中央の縦長枠 → 右中の上・下 → 右の縦長枠 → 下段の左・右」です。9枠固定で、本文は短い2〜4行、見出しは2行までを目安にします。ジャーニーは4〜5段階・5〜6観点を目安にし、増える場合はフェーズで分割します。

詳しい記法、キャンバスの項目順、作例の根拠と検証の扱いは [ビジネス用テンプレートのガイド](docs/BUSINESS_TEMPLATES.md) を参照してください。配色は資料全体のテーマに連動します。
...
| 27 | 大きな画像：1枚を主役に | `figure no-title captioned` |
...
| No. | パターン | クラス |
|---|---|---|
| 42 | 07. ビジネス設計 | `section` |
| 43 | カスタマージャーニーマップ | `journey` |
| 44 | リーンキャンバス：予約管理サービス | `canvas` |
| 45 | ビジネスモデルキャンバス | `canvas` |
| 46 | SWOT：内外の要因を分ける | `cards card-top` |
| 47 | クロスSWOT：要因から施策を導く | `matrix matrix-neutral` |
| 48 | 競合分析：顧客の選択肢を比べる | `comparison-table` |
| 49 | 施策の優先順位：根拠と工数をそろえる | `dense` |
...
| 51 | 提案・意思決定 | `decision` |
| 52 | 次のアクション | `action` |
| 53 | 課題・対応・判断条件を1枚にまとめる | `risk-action` |
| 54 | 参考資料・リンク | `references` |
| 55 | Thank you | `closing` |
